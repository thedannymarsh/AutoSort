...
os.chdir('R:\\Autobots Roll Out\\%s\\JSON_Files' %username)
...
for name in filelist: # For all files in directory
    if name.endswith('.json'): # If the file is a json file
        ...
        with open('R:\\Autobots Roll Out\\%s\\JSON_Files\\%s' % (username, name), "r") as read_file:   
        ...
        for n in range(len(allEvents)):
            ...
            tempVar2 = str.split(tempVar1, ', ') # Split event timestamps whereever ", " exists
            ...
            nex.Rename(doc, doc["Event"], evtName) # Change name of event to current event name    
        print(name)  
        ...
        for n in range(len(allSpikes)):
            ...
            tempVar2 = str.split(tempVar1, ', ') # Split spike timestamps whereever ", " exists
            spkTimes = [] # To hold float type timestamps
            b = [0]*len(tempVar2)
            for q in range(len(tempVar2)): # For all timestamps
                spkTimes.append((float(tempVar2[q])+20)/40000) # Convert from string to float. Then add 20 (for time before spike onset) and divide by 40,000 kHz frequency
            ...
            nex.Rename(doc, doc["Spike"], spkName) # Change name of neuron to current spike name
        ...
        #for n in range(len(allWaves)):
            ...
            for w in range(2,len(tempVar0)): # For all waveforms
                ...
                tempVar2 = str.split(tempVar1, ', ') # Split waveform points whereever ", " exists
                wavePoints = [] # To hold float type points
                for q in range(len(tempVar2)): # For all waveform points
                    wavePoints.append(float(tempVar2[q])/1000) # Convert from string to float. Then divide by 1,000 to convert from V to mV
                waves[w-2] = [b[w-2][1],wavePoints] # Append the timestamp indexing and all waveforms for neuron
            waves.sort()
            for q in range(len(waves)):
                waves[q] = waves[q][1]
            try:
                doc.CreateWaveformVariable(waveName, 40000, spkTimes, waves) # Create waveform variable
            except: continue
